...
void receive_ble_meter_ffff(BLEAdvertisedDevice advertisedDevice)
{
  ...
  // printf("bleffff_meter: seq=%d temperature=%4.1f humidity=%4.1f pressure=%6.1f vbat=%5.3f\n", seq, temperature, humidity, pressure, vbat);
...
void ble_scan_start()
{
  if(ble_scan_state==SCANNING) return;
  ...
  pBLEScan->setWindow(BLE_SCAN_INTERVAL-1);
  #define BLE_SCAN_SEC 5
  if(pBLEScan->start(BLE_SCAN_SEC, &scanCompleteCB, false)){
...
void loop()
{
  ...
  switch(state){
    ...
    case READY_1:
      ...
      if (M5.BtnA.wasPressed() ){ publish_button("A", "wasPressed"); }
      if (M5.BtnA.wasReleased()){ publish_button("A", "wasReleased"); }
      if (M5.BtnB.wasPressed() ){ publish_button("B", "wasPressed"); }
      if (M5.BtnB.wasReleased()){ publish_button("B", "wasReleased"); }

      static unsigned long last_sec10 = -1;
      unsigned long sec10 = (millis()-ready_ms) / 100;
      if (last_sec10 != sec10)
      {
        if(1){
      ...
      if (M5.Axp.GetBtnPress() == 0x01){ // long press (>1s)
        publish_button("P", "long_press");
      }
      if (M5.Axp.GetBtnPress() == 0x02){ // short press (<1s)
